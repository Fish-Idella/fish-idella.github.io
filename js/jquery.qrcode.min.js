	...
	r.qrcode = function(h) {
		...
		h);
			...
			if ("canvas" == h.render) {
				...
				c.width = h.width + 20;
				c.height = h.height + 20;
				var d = c.getContext("2d"), 
				b = h.width / a.getModuleCount(), 
				e = h.height / a.getModuleCount();
				d.fillStyle = h.background;
				d.fillRect(0, 0, c.width, c.height);
				for (var f = 0; f < a.getModuleCount(); f++) for (var i = 0; i < a.getModuleCount(); i++) {
					d.fillStyle = a.isDark(f, i) ? h.foreground: h.background;
					var g = Math.ceil((i + 1) * b) - Math.floor(i * b),
					j = Math.ceil((f + 1) * b) - Math.floor(f * b);
					d.fillRect(Math.round(i * b + 10), Math.round(f * e + 10), g, j);
				}
				if (h.logo) {try{d.drawImage(h.logo, c.width/2-30, c.height/2-30, 60, 60);}catch(e){}}
			} else {
				a = new o(h.typeNumber, h.correctLevel);
				a.addData(h.text);
				a.make();
