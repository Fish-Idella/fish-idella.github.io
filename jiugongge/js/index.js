(function (HJD) {
    ...
    (function getMainURL(url, i, max) {
        ...
        }).catch(() => setTimeout(() => getMainURL(HJD.navs[i % max], i + 1, max), 1000));
    }((window.localStorage.getItem('main-url') || HJD.base), 1, HJD.navs.length));

}({

    // fid-7.html
    base: "http://doww.elsbhqdzqad.com/w33.php",
    ...
    loadMainList: function (url, title) {
        ...
        if (!this.loading) {
            ...
            fetch(url).then(r => r.text())
                .then(html => PuSet.parseHTML(html).querySelectorAll("#ajaxtable tr > td:nth-child(2) > a.subject").forEach(elem => this.elems.main.appendChild(elem)))
